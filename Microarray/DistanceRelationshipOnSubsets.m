% check coecpression - dictance relationship on a subset of samples
close all;
clear all;
...
%% calculate fit curve for all samples
[curve, goodness, output] = fit(DistExpVect(:,1),DistExpVect(:,2),'poly2');
...
for t=1:10
NumOfSamples = 100; 
r = randi([1 1284],1,NumOfSamples);
samp(:,t)=r;
SubsetExp = SampleCoexpression(r,r);
SubsetDist = MRIvoxCoordinates(r,r);

VectSubset(:,1) = SubsetDist(:);
% replace diagonal with zeros
VectSubset(:,2) = SubsetExp(:);
VectSubset( ~any(VectSubset,2), : ) = [];  %rows
