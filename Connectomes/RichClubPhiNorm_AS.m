function [PhiNorm,PhiTrue,PhiRand, rAdj] = RichClubPhiNorm_AS(Adj,smax,numIter,numRepeats,WhatTypeNetwork,whatNullModel, wei_freq)
% RichClubPhiNorm
%
% INPUTS:
...
case 'wd' % Weighted, directed network:
    RichClubFun = @(x) rich_club_wd(x,smax);
case 'wu' % Weighted, undirected network:
    RichClubFun = @(x) rich_club_wu(x,smax);
case 'wus' % Weighted, undirected network:
    RichClubFun = @(x) rich_club_wu_strength(x,smax);
case 'bu2h' % Weighted, undirected network:
    RichClubFun = @(x) rich_club_bu(x,smax);
...
case 'bd' % binary, directed network:
    ...
    RichClubFun = @(x) rich_club_bd(x,smax);
otherwise
